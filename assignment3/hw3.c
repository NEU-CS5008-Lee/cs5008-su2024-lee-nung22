...
queue_t *newQueue()
{
  ...
  // if queue was created successfully, initialize head and tail pointers to NULL
  if (q_p != NULL)
  {
...
bool isEmpty(queue_t *q_p)
{
  ...
  // if head pointer is not NULL, then queue is not empty
  if (q_p->head_p != NULL)
  {
    b = false;
  }
...
void enqueue(queue_t *q_p, int d)
{
  ...
  if (q_p != NULL)
  {
    ...
    if (isEmpty(q_p))
    {
      ...
      // create new node and set head and tail pointers to it
      q_p->head_p = q_p->tail_p = newNode(d);
    }
    else
    {
      // queue is not empty
      ...
      // create a new node and add it to the tail of the queue
      n_p = q_p->tail_p;
      q_p->tail_p = newNode(d);
