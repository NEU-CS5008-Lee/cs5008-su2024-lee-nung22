...
void heapify(pq *p2, int n, int i)
{
  ...
  // Check if the left child has higher priority than the current largest node
  if (lchild < n && p2->heap[lchild].priority > p2->heap[largest].priority)
  {
    largest = lchild;
  }

  // Check if the right child has higher priority than the current largest node
...
void shiftUp(pq *p2, int i)
{
  ...
  // Continue swapping the current node with its parent as long as the current node has higher priority
