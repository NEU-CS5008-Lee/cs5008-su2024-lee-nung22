...
void *philosopher(void *x)
{
  ...
  while (1)
  {
    ...
    sleep(n % 2); // Introduce a delay based on the philosopher's index to simulate non-blocking behavior where philosophers don't all start at the same time

    // Pick up chopsticks (mutexes)
    int left = n;
    int right = (n + 1) % 5;

    // To avoid deadlock, ensure that all philosophers pick up the lower numbered chopstick first
    if (n % 2 == 0)
    {
