...
/* A function to create a graph with an array of adjacency lists which is = number of vertices */
...
void addConnection(Graph *graph, char *person, char *friend)
{
  ...
  // If person not in graph, add person
  if (p == -1)
  {
    p = n; // position for the new person
    graph->adjLists[p] = createNode(person);
    n++;
  }

  // If friend not in graph, add friend
  if (f == -1)
  {
    f = n; // position for the new friend
    graph->adjLists[f] = createNode(friend);
  }

  // Add friend to person's list
  node *newFriendNode = createNode(friend);
  newFriendNode->next = graph->adjLists[p]->next;
  graph->adjLists[p]->next = newFriendNode;

  // Add person to friend's list
  node *newPersonNode = createNode(person);
  newPersonNode->next = graph->adjLists[f]->next;
  graph->adjLists[f]->next = newPersonNode;
}
