...
void partition(int s[], int n, int k)
{
  ...
  /* 2 to k partitions */
  for (j = 2; j <= k; j++) /* for each partition count from 2 to k */
  {
    for (i = 2; i <= n; i++) /* for each book from 2 to n */
    {
      m[i][j] = MAXINT; /* set initial cost to infinity */
      for (x = 1; x < i; x++) /* for each possible partition point */
      {
        cost = max(m[x][j - 1], p[i] - p[x]); /* calculate cost */
        if (m[i][j] > cost) /* if current cost is greater than the calculated cost */
        {
          m[i][j] = cost; /* update cost */
          d[i][j] = x; /* store partition point */
        }
      }
    }
  }
