...
int main()
{
  ...
  while (!isEmpty(q))
  {
    ...
    current = dequeue(q); // get the current node from the queue
    if (!done[current])
    {
      printf("%d ", current); // print the current node
      done[current] = true;   // mark the current node as done

      // enqueue all adjacent nodes that have not been processed
      for (j = 0; j < GSIZE; j++)
      {
